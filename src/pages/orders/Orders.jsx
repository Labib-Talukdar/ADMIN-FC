...
import React, { useEffect, useState } from 'react';
import API from '../../api'; // তোমার API Instance এর পাথ
import { 
  HiOutlineCheckCircle, 
  HiOutlineClock, 
  HiOutlineTrash, 
...
const Orders = () => {
  ...
  // ৩. পেন্ডিং বাটন ক্লিক করলে অর্ডার কনফার্ম করা
  const handleApproveOrder = async (orderId) => {
    try {
      await API.put(`/api/orders/${orderId}/status`, { status: 'confirmed' });
...
const handleDeleteOrder = async (orderId) => {
  ...
  try {
    ...
    const errorMessage = error.response?.data?.message || "অর্ডার মুছে ফেলতে সমস্যা হয়েছে!";
    alert(errorMessage);
  }
};
  ...
  // ৫. ট্যাব কাউন্ট এবং ফিল্টারড লিস্ট
  const pendingCount = orders.filter(o => getNormalizedStatus(o.status) === 'pending').length;
  const confirmedCount = orders.filter(o => getNormalizedStatus(o.status) === 'confirmed').length;
