...
const EditProduct = () => {
  ...
  const [inStock, setInStock] = useState(true);
  const [selectedSizes, setSelectedSizes] = useState([]);
  const [customSize, setCustomSize] = useState(""); // 👈 কাস্টম সাইজের লোকাল স্টেট
  const [selectedColors, setSelectedColors] = useState([]);
  const [mainImage, setMainImage] = useState(null);
  const [existingImage, setExistingImage] = useState("");
    ...
    <div className="max-w-4xl mx-auto px-4 py-10 font-sans text-gray-800">
      ...
      {loading ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-6 border border-gray-100 p-6 sm:p-8 bg-white rounded-2xl shadow-lg">
          ...
          {/* 🌟 Sizes Section (Updated with Custom Add Option) */}
          <div className="p-5 bg-gray-50 rounded-xl border border-gray-200/80 space-y-4">
            <label className="block text-xs font-semibold uppercase tracking-wider text-gray-600">
              Available Sizes
            </label>

            {/* ১. কুইক সিলেক্ট ডিফল্ট সাইজ */}
            <div>
            ...
            {/* ২. কাস্টম সাইজ টাইপ করার ইনপুট বক্স */}
            <div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-4 py-2 border border-gray-300 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-teal-600 text-gray-800"
                  onKeyDown={(e) => {
                    if (e.key === "Enter") {
                      e.preventDefault();
                ...
                <button
                  ...
                  className="px-4 py-2 bg-[#022525] hover:bg-teal-800 text-white font-bold text-xs rounded-lg transition uppercase tracking-wider"
                >
                  + Add Size
                </button>
              </div>
            ...
            {/* ৩. সিলেক্টেড সমস্ত সাইজ দেখানোর জায়গা */}
              ...
              <div className="pt-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedSizes.map((s) => (
                    <span
                      key={s}
                      className="inline-flex items-center gap-1.5 px-3 py-1 bg-teal-100 text-teal-900 font-bold text-xs rounded-lg border border-teal-200"
                    >
                      {s}
                      <button
                        type="button"
                        onClick={() => handleRemoveSize(s)}
                        className="text-teal-700 hover:text-red-600 font-black text-sm leading-none ml-1"
                        title="Remove size"
                      >
                        ×
                      </button>
